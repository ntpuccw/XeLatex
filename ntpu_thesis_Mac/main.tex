...
\newpage
\include{ch0_thanks}   % 謝誌
\include{ch0_abstract}   % 中英文摘要
...
\setcounter{page}{1}
\include{ch1_intro}             % CH1: introduction
\ifodd\count0 \else \thispagestyle{empty}\mbox{}\clearpage\fi          % 如果是偶數頁就呈現空白頁
\include{ch2_dist}                              % CH2: distribution
\ifodd\count0 \else \thispagestyle{empty}\mbox{}\clearpage\fi
\include{ch3_model}                             % CH3: modeling
\ifodd\count0 \else \thispagestyle{empty}\mbox{}\clearpage\fi
\include{ch4_simu}                              % CH4: simulation
\ifodd\count0 \else \thispagestyle{empty}\mbox{}\clearpage\fi
\include{ch5_summary}                           % CH5 summary and suggestion
